...
sql_help_ALTER_TABLE(PQExpBuffer buf)
{
	appendPQExpBuffer(buf,
					  ...
					  "    ALTER [ COLUMN ] %s SET STATISTICS DISTINCT %s\n"
...
{
	appendPQExpBuffer(buf,
					  "CREATE [ PROCEDURAL ] LANGUAGE %s\n"
					  "CREATE [ TRUSTED ] [ PROCEDURAL ] LANGUAGE %s\n"
...
sql_help_CREATE_TABLE(PQExpBuffer buf)
{
	appendPQExpBuffer(buf,
					  ...
					  "  { %s %s [ DEFAULT %s ] [ %s [ ... ] ]\n"
					  "    | %s\n"
					  "    | LIKE %s [ { INCLUDING | EXCLUDING } { DEFAULTS | CONSTRAINTS | INDEXES | STORAGE | COMMENTS | ALL } ] ... }\n"
					  "    [, ... ]\n"
					  "] )\n"
					  "[ INHERITS ( %s [, ... ] ) ]\n"
					  ...
					  _("action"),
					  _("action"),
					  _("index_parameters in UNIQUE and PRIMARY KEY constraints are:"),
					  _("storage_parameter"),
					  _("value"),
					  _("tablespace"));
}
...
{
	appendPQExpBuffer(buf,
					  "EXPLAIN [ ( { ANALYZE %s | VERBOSE %s | COSTS %s | FORMAT { TEXT | XML | JSON } } [, ...] ) ] %s\n"
